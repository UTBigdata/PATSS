...
class search {
  ...
  def main(args: Array[String]): Unit = { //"384" "/global_staTroOri0_sucReal" "0.6" "/ours/queryBatchAll-0.01" "192" "spark://10.103.104.102:7077"
...
//      ###8###7###40  CH CS L
      ...
      val Q_canSigArr = new candiSig().searchGet(Q_sigSeg, Ql, QH, QS, J)
      val q_canSigArrBC = spark.sparkContext.broadcast(Q_canSigArr)

      val searchResCan = relTra.map { T =>
